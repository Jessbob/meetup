...
describe("<Event /> render", () => {
    ...
    EventWrapper = shallow(
      <Event
        event={{
          ...
          yes_rsvp_count: 90,
          link:
            "https://www.meetup.com/Kotlin-User-Group-Munich/events/260445923/",
          description:
            "<p>Hello Kotliners. Join us in our May gathering at Google office.</p> <p>Agenda:</p> <p>19:00 - Opening doors</p> <p>19:15 - Michal Harakal - Kotlin multiplatform roller-coaster ride or how I've rewritten a conference app over the weekend.</p> <p>19.55 - Shagen Ogandzhanian (JetBrains) - Kotlin and JS interop, what's next. How to use javascript libraries in Kotlin world</p> <p>20.30 - Leonid Popescu (Payworks) - Hybrid SDKs or how we started with Kotlin/Multiplatform without trashing the existing code base</p> <p>Have you been working on a project with Kotlin? Is there a library you would like to discuss? Do you want to try public speaking?<br/>Message us, come and have fun sharing your Kotlin story!</p>",
          visibility: "public",
          venue: {
            name: "Google",
  ...
  test("render correct information", () => {
    ...
    expect(EventWrapper.find(".name").text()).toEqual(
      "Kotlin Users Group Meetup May"
    );
    expect(EventWrapper.find(".going").text()).toEqual("90 people are going");
  });

  test("show extra info when user clicks Details button", () => {
    EventWrapper.find(".details-btn").simulate("click");
    expect(EventWrapper.find(".extra")).toHaveLength(1);
  });

  test("hide extra info when details button is clicked", () => {
    EventWrapper.setState({ expanded: true });
    EventWrapper.find(".details-btn").simulate("click");
    expect(EventWrapper.find(".extra")).toHaveLength(0);
  });

  test("show correct extra info", () => {
    EventWrapper.setState({ expanded: true });
    expect(EventWrapper.find(".extra .address").text()).toEqual(
      "Google, Erika-Mann-Straße 33, München, Germany"
    );
    expect(EventWrapper.find(".extra .description").html()).toEqual(
      "<div class=\"description\"><p>Hello Kotliners. Join us in our May gathering at Google office.</p> <p>Agenda:</p> <p>19:00 - Opening doors</p> <p>19:15 - Michal Harakal - Kotlin multiplatform roller-coaster ride or how I've rewritten a conference app over the weekend.</p> <p>19.55 - Shagen Ogandzhanian (JetBrains) - Kotlin and JS interop, what's next. How to use javascript libraries in Kotlin world</p> <p>20.30 - Leonid Popescu (Payworks) - Hybrid SDKs or how we started with Kotlin/Multiplatform without trashing the existing code base</p> <p>Have you been working on a project with Kotlin? Is there a library you would like to discuss? Do you want to try public speaking?<br/>Message us, come and have fun sharing your Kotlin story!</p></div>"
    );
